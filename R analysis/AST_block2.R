...
# Install packages with BiocManager if they are not already installed
if (!require("BiocManager", quietly = TRUE))
  install.packages("BiocManager")

BiocManager::install(c("DESeq2","genomation","GenomicFeatures"))

# -------------------------------------------------------------------------------------------
# Load all packages
pkgs <- c("tidyverse", "plyr", "Rfast", "tryCatchLog", 
          "lmerTest", "lme4", "car", "gridExtra", 
          "doParallel", "ggpubr", "grid", "tagcloud", 
          "ggprism","DESeq2","genomation","GenomicFeatures")
invisible(lapply(pkgs, function(x) library(x, character.only=TRUE)))


# Load custom functions from the project repository
source("~/scratch/AST/AST-tutorial/AST_functions.R")

# --------------------------------------------------------------

# Sample table
metadata <- read.csv("~/scratch/AST/metadataBlock2.csv")

# -------------------- 3.3 GENERATE SAMPLE X SNP:GENE COUNT MATRIX ------------------------------

# ------------------ 3.3.1 Filter SNPs by transcript ---------------
# Load in filter list
filterlist <- read.csv("~/scratch/AST/AST-tutorial/genelist_filter.csv",header=F)[,c(1)]

# Make txdb from the honey bee gene annotations
txdb <- makeTxDbFromGFF("~/scratch/AST/INDEX/Amel_HAv3.1.gff")
transcripts <- transcripts(txdb)

# function for filtering SNPs (those within miRNA, tRNA, repetitive pseudogenes)
...
SNPs <- filter_SNPs("~/scratch/AST/ANALYSIS_SETS/Block2_SNPs_for_Analysis_Sorted.bed",
                    transcripts,filterlist)

# Save output
write.csv(SNPs,"~/scratch/AST/ANALYSIS/INTERMEDIATE/Block2_SNPs.csv",row.names=F)

# ---------------- 3.3.2 merge count files to matrix ----------------------------

# function to import F2 read counts at F1 SNPs generated from section 2 to construct a count matrix
make_ASE_counts_matrix <- function(DIR_COUNTS,cores,SNPs){
  ...
  DIR_COUNTS="~/scratch/AST/COUNT/BLOCK2"
  cores=4
  # Read in data
...
# Execute function
SNP_counts <- make_ASE_counts_matrix("~/scratch/AST/COUNT/BLOCK2",4,SNPs)
# order the columns of SNP_counts to be the same as your metadata$sample.id column
SNP_counts <- SNP_counts[, match(metadata$sample.id, colnames(SNP_counts))]

# Save output
write.csv(SNP_counts,"~/scratch/AST/ANALYSIS/INTERMEDIATE/Block2_SNP_gene_counts.csv")

# ---------------  3.4 PREPARE SAMPLE X SNP:GENE COUNTS FOR ANALYSIS -------------------

# ------ 3.4.1 normalize by library size -------------

# To adjust for differences in sequencing depth between libraries, 
# library size factors are estimated and the read counts are normalized using
# the median of ratios normalization (MRN) method from DESeq2.

# -------First, calcSizeFactors sums the parent-specific counts by library and estimates library size factors:
...
# Execute
SNP_counts_normalized <- normalizeASReadCounts(SNP_counts,size_factors)

# Save output
write.csv(SNP_counts_normalized,"~/scratch/AST/ANALYSIS/INTERMEDIATE/Block2_SNP_gene_counts_normalized.csv")

# ------- 3.4.2 Split by phenotype and filter low count SNPs---------

GN.IDs <- metadata[metadata$phenotype=="GN","sample.id"]
GN_counts_normalized <- SNP_counts_normalized[,names(SNP_counts_normalized)%in%GN.IDs]
GN_counts <- SNP_counts[row.names(SNP_counts)%in%row.names(GN_counts_normalized),
                        names(SNP_counts)%in%GN.IDs]

BN.IDs <- metadata[metadata$phenotype=="BN","sample.id"]
BN_counts_normalized <- SNP_counts_normalized[,names(SNP_counts_normalized)%in%BN.IDs]
BN_counts <- SNP_counts[row.names(SNP_counts)%in%row.names(BN_counts_normalized),
                        names(SNP_counts)%in%BN.IDs]

# SNPs with n<1 count in any Lineage are discarded. Finally, genes with n<2 SNPs are discarded, 
# as this method requires at least two independent observations per gene for statistical analysis.
...
# Execute
GN_counts_normalized <- filter_counts(GN_counts_normalized,0)
GN_counts <- GN_counts[row.names(GN_counts)%in%row.names(GN_counts_normalized),]
BN_counts_normalized <- filter_counts(BN_counts_normalized,0)
BN_counts <- BN_counts[row.names(BN_counts)%in%row.names(BN_counts_normalized),]

# Save output
write.csv(GN_counts_normalized,"~/scratch/AST/ANALYSIS/INTERMEDIATE/Block2_GN_counts_normalized.csv")
write.csv(GN_counts,"~/scratch/AST/ANALYSIS/INTERMEDIATE/Block2_GN_counts.csv")
write.csv(BN_counts_normalized,"~/scratch/AST/ANALYSIS/INTERMEDIATE/Block2_BN_counts_normalized.csv")
write.csv(BN_counts,"~/scratch/AST/ANALYSIS/INTERMEDIATE/Block2_BN_counts.csv")
...
BN.SK <- AST.SK(BN_counts_normalized,"BN",4)

# Save output
write.csv(GN.SK,"~/scratch/AST/ANALYSIS/RESULTS/Block2_GN_SK.csv", row.names=F)
write.csv(BN.SK,"~/scratch/AST/ANALYSIS/RESULTS/Block2_BN_SK.csv", row.names=F)

# --------- 3.5.2 Gene-level general linear mixed models (GLIMMIX)
# For each gene, GLIMMIX assess the effects of Parent, Lineage, and their interaction on the raw read counts at each SNP, 
# using the log of the library size factors as an offset to adjust for variation in sequencing depth between libraries.
AST.GLIMMIX <- function(counts,size_factors,cores){
  ...
  df.out <- foreach(i=1:i.len,.combine=rbind) %dopar% {
    ...
    testfail <- F
    test <- "null"
    tryCatchLog(test <- lmerTest::lmer(count~parent+lineage+parent*lineage+(1|SNP_gene)+
...
# Execute
GN.GLIMMIX <- AST.GLIMMIX(GN_counts,size_factors,6)
BN.GLIMMIX <- AST.GLIMMIX(BN_counts,size_factors,6)

# Save output
write.csv(GN.GLIMMIX,"~/scratch/AST/ANALYSIS/RESULTS/Block2_GNGLIMMIX.csv", row.names=F)
write.csv(BN.GLIMMIX,"~/scratch/AST/ANALYSIS/RESULTS/Block2_BNGLIMMIX.csv", row.names=F)
...
# Execute
GN.plot <- AST.Analysis(GN_counts_normalized,
                        "GN",GN.SK,GN.GLIMMIX)
BN.plot <- AST.Analysis(BN_counts_normalized,
                        "BN",BN.SK,BN.GLIMMIX)

# Save output
write.csv(GN.plot,"~/scratch/AST/ANALYSIS/RESULTS/Block2_GNAST.csv",row.names=F)
write.csv(BN.plot,"~/scratch/AST/ANALYSIS/RESULTS/Block2_BNAST.csv",row.names=F)


# ---------- 3.6.2 Chi-squared tests

allgenes <- read.table("~/scratch/AST/INDEX/Amel_HAv3.1_geneIDs.txt",header=F)[,1]
allgenes <- setdiff(allgenes,filterlist)
...
# Execute
chisq.df <- AST.chisq("GN",GN.plot,"BN",BN.plot,allgenes)

# ---------------- 3.7 Data Visualization

# First, reorder the data generated by AST.Analysis so that unbiased genes appear first:
GN.plot <- rbind(GN.plot[GN.plot$bias.plot%in%c("NA"),],
                 GN.plot[GN.plot$bias.plot%in%c(
                   "mat", "Lineage A", "Lineage B", "pat"),])
GN.plot$bias.plot <- factor(GN.plot$bias.plot,
                            levels = c("NA","mat", "Lineage A", 
                                       "Lineage B", "pat"))

BN.plot <- rbind(BN.plot[BN.plot$bias.plot%in%c("NA"),],
                 BN.plot[BN.plot$bias.plot%in%c(
                   "mat", "Lineage A", "Lineage B", "pat"),])
BN.plot$bias.plot <- factor(BN.plot$bias.plot,
                            levels = c("NA","mat", "Lineage A", 
                                       "Lineage B", "pat"))

# Next, this function collapses the data generated by AST.Analysis and calculates p1 and p2 by gene
...
# Execute
GN.collapse <- AST.collapse(GN.plot,GN_counts_normalized,"GN")
BN.collapse <- AST.collapse(BN.plot,BN_counts_normalized,"BN")

# Save output
write.csv(GN.collapse,"~/scratch/AST/ANALYSIS/RESULTS/Block2_GNAST_collapse.csv",row.names=F)
write.csv(BN.collapse,"~/scratch/AST/ANALYSIS/RESULTS/Block2_BNAST_collapse.csv",row.names=F)

# the actual goddam scatter plots
...
fig2 <- arrangeGrob(GN.scatter, chisq.table, BN.scatter, widths=c(5,2.5,5))
plot(fig2)
